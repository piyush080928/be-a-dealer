...
function CarCard() {
  const cars = Gamedata.filter(item => item.category === "CAR");
    ...
    <div className="flex flex-col gap-10 px-4 py-6">
      ...
      <div className="flex flex-wrap justify-center gap-6">
        {cars.map((c) => (
          <div
            key={c.id}
            className="w-full sm:w-[300px] md:w-[360px] lg:w-[400px] shadow-md shadow-black/50 hover:shadow-lg cursor-pointer transition-all overflow-hidden bg-gray-300 dark:bg-gray-800 rounded-lg flex flex-col"
          >
            <div className="relative">
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-black/50 backdrop-blur-[1px] p-2">
                <p className="text-white text-xs sm:text-sm">{c.description}</p>
              </div>
            </div>
